/*----------------------------------------------------------------------------
 * Name:    Blinky.c
 * Purpose: Dual Core LED Flasher for LPC54102-LPCXpresso  (M4 part)
 *----------------------------------------------------------------------------
...
#define M0plus_IMAGE_START    0x40000          /* M0+ Image located in flash  */


volatile uint32_t ledState;    /* must be global because we use it's addresss */
...
void MAILBOX_IRQHandler(void) {
  ...
  LED_SetOut(ledState);                       /* output the LED state         */
}
...
int main(void) {
  ...
  ledState = 1;                               /* value is changed from M0+    */
  ...
  while (1) {
    ...
    if (M0plus_Trigger) {
      ...
      MAILBOX_mutexWait();
      LPC_MBOX->BOX[0].IRQ = (uint32_t)&ledState; /* signal address to M0+       */
      MAILBOX_mutexRelease();
    }
  }
