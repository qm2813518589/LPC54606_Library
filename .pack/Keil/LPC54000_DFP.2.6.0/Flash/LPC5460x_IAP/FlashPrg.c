 ...
 * $Revision:    V1.00
 *
 * Project:      Flash Device Algorithm for NXP LPC5460x Flash using IAP
 * --------------------------------------------------------------------------- */
...
#ifdef LPC5460X
  #define MEMMAP       (*((volatile unsigned char *) 0x40000000))
#endif

// Main Clock
#ifdef LPC5460X
  #define MAINCLKSELA  (*((volatile unsigned long *) 0x40000280))
  #define MAINCLKSELB  (*((volatile unsigned long *) 0x40000284))
  #define AHBCLKDIV    (*((volatile unsigned long *) 0x40000380))
#endif
...
int Init (unsigned long adr, unsigned long clk, unsigned long fnc) {
  ...
  MAINCLKSELA = 0;
  MAINCLKSELB = 0;
  AHBCLKDIV   = 0;                             // Set Main Clock divider to 1
...
unsigned long Verify (unsigned long adr, unsigned long sz, unsigned char *buf) {
  ...
  IAP.cmd    = 56;                             // Compare <address1> <address2> <no of bytes>
  IAP.par[0] = adr;                            // Destination Flash Address
  IAP.par[1] = (unsigned long)buf;             // Source RAM Address
  IAP.par[2] = (sz + 3) & ~(3); //1024;                           // Fixed Page Size
  IAP.par[3] = CCLK;                           // CCLK in kHz
  IAP_Call (&IAP.cmd, &IAP.stat);              // Call IAP Command
  if (IAP.stat) return (adr);                  // Command Failed
